  ...
  return (
    <DashboardShell>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
          ...
          {(['all', 'active', 'visitor', 'inactive'] as StatusFilter[]).map((st) => (
            <button
              ...
              className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-semibold capitalize transition-all flex-shrink-0 ${
                selectedStatus === st
      ...
      <div className="glass-card overflow-hidden">
        {loading ? (
          <div className="p-10 text-center text-white/30 text-sm">Loading members directory...</div>
          ...
          <div className="overflow-x-auto w-full">
            <table className="w-full text-xs sm:text-sm text-left">
              <thead>
                <tr className="border-b border-white/6 whitespace-nowrap">
                  <th className="px-3.5 sm:px-4 py-3 text-[10px] sm:text-[11px] font-medium text-white/30 uppercase tracking-wider">Member Name</th>
                  <th className="px-3.5 sm:px-4 py-3 text-[10px] sm:text-[11px] font-medium text-white/30 uppercase tracking-wider">Phone Number</th>
                  <th className="px-3.5 sm:px-4 py-3 text-[10px] sm:text-[11px] font-medium text-white/30 uppercase tracking-wider">Joined Date</th>
                  <th className="px-3.5 sm:px-4 py-3 text-[10px] sm:text-[11px] font-medium text-white/30 uppercase tracking-wider">Status</th>
                </tr>
              </thead>
                    ...
                    <tr key={member.id} className="table-row-hover animate-fade-in" style={{ animationDelay: `${idx * 15}ms` }}>
                      <td className="px-3.5 sm:px-4 py-3">
                        <div className="flex items-center gap-2.5 sm:gap-3">
                          <div className="w-7 sm:w-8 h-7 sm:h-8 rounded-full flex items-center justify-center text-xs font-semibold text-navy-dark flex-shrink-0"
                            style={{ background: 'linear-gradient(135deg, #C9A84C, #8B6914)' }}>
                            {(member.full_name ?? '?')[0]?.toUpperCase() ?? '?'}
                          </div>
                          <span className="font-medium text-white text-xs sm:text-sm">{member.full_name ?? 'Unknown Member'}</span>
                        </div>
                      </td>
                      <td className="px-3.5 sm:px-4 py-3">
                      ...
                      <td className="px-3.5 sm:px-4 py-3">
                        <div className="relative inline-block">
                          <select
                            ...
                            className={`px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-semibold capitalize border focus:outline-none cursor-pointer transition-all ${
                              isActive
      ...
      {!loading && filteredMembers.length > 0 && (
        <div className="mt-3 text-xs text-white/30 px-1 text-right">
