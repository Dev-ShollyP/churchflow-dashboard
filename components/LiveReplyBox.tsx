    ...
    <div className="mt-4 glass-card p-4">
      ...
      <form onSubmit={handleSend} className="flex items-center gap-3">
        <input
          ...
          disabled={sending}
          className="flex-1 px-4 py-2.5 rounded-xl text-sm text-white placeholder-white/30 bg-navy-dark/60 border border-white/10 focus:border-gold/50 focus:outline-none transition-colors"
        />

        <button
          type="submit"
          disabled={sending || !replyText.trim()}
          className="px-5 py-2.5 rounded-xl text-xs font-semibold text-navy-dark transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2 flex-shrink-0 shadow-gold"
