...
// Read the base workflow from user or existing
...
// 1. Build the complete, clean JavaScript code for "Build AI Context" node
const buildAiContextJsCode = `const settingsItems = $("Get Branch Settings").all();
...
// ── 0. NON-FAITH / OFF-TOPIC GUARDRAIL DIRECT INTERCEPT ──
...
if (looksLikeHymnRequest && hymnsDb.length > 0) {
  ...
  if (!matchedHymn) {
    const recentItems = $("Fetch Recent Messages").all();
...
if (buildAiNode && buildAiNode.parameters) {
  buildAiNode.parameters.jsCode = buildAiContextJsCode;
}

// Write the complete workflow JSON file
fs.writeFileSync(workflowPath, JSON.stringify(workflow, null, 2), 'utf8');
console.log('Successfully generated complete updated n8n-workflow-churchflow-fixed.json!');
