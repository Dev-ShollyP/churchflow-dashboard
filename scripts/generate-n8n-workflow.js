      ...
      "parameters": {
        ...
        "queryParameters": {
          "parameters": [
            {
              "name": "or",
              "value": "=(branch_id.eq.{{$(\"Extract branch_id\").first().json.branch_id}},branch_id.is.null)"
            },
            {
              "name": "order",
...
function resolvePublicFlyerUrl(id, rawFlyer, fallback) {
  if (rawFlyer && (rawFlyer.startsWith('http://') || rawFlyer.startsWith('https://')) && rawFlyer.indexOf('data:image/') === -1) {
    return rawFlyer;
  }
  if (id && String(id).indexOf('recurring-') !== 0) {
    return 'https://churchflow-dashboard.vercel.app/api/events/flyer?id=' + id;
  }
  return fallback || (storageBase + '/Service/First%20Service.jpg');
}
...
if (requestedMonth) {
  ...
  if (allMonthItems.length > 0) {
    ...
    let firstFlyer = storageBase + '/Service/First%20Service.jpg';
    if (monthPrograms.length > 0) {
      firstFlyer = resolvePublicFlyerUrl(monthPrograms[0].id, monthPrograms[0].flyer_url || monthPrograms[0].image_url);
    } else if (monthEvents.length > 0) {
      firstFlyer = resolvePublicFlyerUrl(monthEvents[0].id, monthEvents[0].banner_url);
    }
...
if (matchedDbEvent) {
  ...
  const flyer = resolvePublicFlyerUrl(matchedDbEvent.id, matchedDbEvent.banner_url || meta.embeddedFlyer);
...
for (var p = 0; p < specialPrograms.length; p++) {
  ...
  if (matchesSpecialProgram(program)) {
    ...
    const flyer = resolvePublicFlyerUrl(program.id, program.flyer_url || program.image_url);
    return [{ json: { is_direct: true, is_interactive: false, image_url: flyer, direct_reply: replyText } }];
  }
}
